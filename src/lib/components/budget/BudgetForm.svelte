<script lang="ts">
	...
	let subcategories: { name: string; budget: number }[] =
		editCategory?.subcategories.map((s) => ({ name: s.name, budget: s.monthlyBudget })) ?? [];

	function addSubcategory() {
		subcategories = [...subcategories, { name: '', budget: 0 }];
	}
