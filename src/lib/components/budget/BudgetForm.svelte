...
<form on:submit|preventDefault={submit} class="space-y-4">
        ...
        {#each subcategories as sub, i}
          <div class="space-y-1">
            <div class="flex gap-2 items-center">
              <input class="input flex-1" type="text" bind:value={sub.name} placeholder="Subcategory name" />
              <input class="input w-28" type="number" step="0.01" bind:value={sub.budget} min="0" placeholder="$0" />
              <button
                type="button"
  ...
  <button type="button" class="btn-ghost text-sm" on:click={addSubcategory}> + Add Subcategory </button>
