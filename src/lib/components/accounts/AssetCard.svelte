...
<div class="card space-y-3">
	<div class="flex items-center justify-between">
		...
		<div class="flex gap-1">
			...
			<button class="btn-ghost p-2 hover:text-red-400" on:click={() => dispatch('delete', asset)} aria-label="Delete">
				<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<div class="grid grid-cols-3 gap-4 text-sm">
		...
		{#if asset.remainingBalance}
			...
			<div>
				...
				<div class="font-semibold {equity >= 0 ? 'text-emerald-400' : 'text-red-400'} tabular-nums">{formatCurrency(equity)}</div>
			</div>
		{/if}
	</div>
</div>
