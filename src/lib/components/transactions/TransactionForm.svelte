<script lang="ts">
	...
	export let defaultAccountId: string = '';
	...
	budgetStore.categories.subscribe((c: BudgetCategory[]) =>
		(budgetCategories = c.slice().sort((a, b) => a.name.localeCompare(b.name)))
	);
...
<form on:submit|preventDefault={submit} class="space-y-3">
	...
	<div>
		<label class="label" for="tx-notes">Notes (optional)</label>
		<input id="tx-notes" class="input" type="text" bind:value={notes} placeholder="Optional notes" />
	</div>
