<script lang="ts">
  ...
  $: assignments = $plannerStore.filter(a => a.plannerMonth === plannerMonth && a.paycheckDate === paycheckDate)
  ...
  $: clearedCount = assignedBills.filter(
    ({ assignment }) => monthTransactions.find(t => t.id === assignment.transactionId)?.clearedStatus === "cleared",
  ).length
