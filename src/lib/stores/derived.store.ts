import type { AssetAccount, LoanAccount } from "$lib/types"
import { derived } from "svelte/store"
import { accountsStore } from "./accounts.store"

export const totalLiquid = derived(accountsStore, $accounts =>
  $accounts.filter(a => a.type === "checking" || a.type === "savings").reduce((sum, a) => sum + a.balance, 0),
)

export const totalLiabilities = derived(accountsStore, $accounts =>
  $accounts.filter(a => a.type === "loan").reduce((sum, a) => sum + (a as LoanAccount).remainingBalance, 0),
)
...
export const netWorth = derived([totalAssets, totalLiabilities], ([$assets, $liabilities]) => $assets - $liabilities)
