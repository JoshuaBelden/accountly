...
function createBudgetStore() {
	...
	const overrideStore = writable<MonthlyBudgetOverride[]>(
		loadFromStorage<MonthlyBudgetOverride[]>(OVERRIDE_KEY, [])
	);
	...
	return {
		...
		updateCategory(id: string, changes: Partial<BudgetCategory>) {
			catStore.update((list) =>
				list.map((c) => (c.id === id ? { ...c, ...changes, updatedAt: now() } : c))
			);
		},
		...
		addSubcategory(parentId: string, sub: BudgetSubcategory) {
			catStore.update((list) =>
				list.map((c) =>
					c.id === parentId
						? { ...c, subcategories: [...c.subcategories, sub], updatedAt: now() }
						: c
				)
			);
		},
